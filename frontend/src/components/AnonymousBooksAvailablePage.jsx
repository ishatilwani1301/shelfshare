...
const AnonymousBooksAvailablePage = ({ searchQuery, onSelectBookOffer }) => {
  const [allFilteredOffers, setAllFilteredOffers] = useState([]); 
  ...
  // Helper functions to get random color and rotation
  ...
  useEffect(() => {
    ...
    setCurrentPage(1);
    setTimeout(() => {
      let filteredOffers = allMockOffers;

      
      if (searchQuery) {
        const lowerCaseSearchQuery = searchQuery.toLowerCase();
        filteredOffers = filteredOffers.filter(offer =>
          offer.title.toLowerCase().includes(lowerCaseSearchQuery) ||
          offer.description.toLowerCase().includes(lowerCaseSearchQuery) ||
          offer.anonymousAuthor.toLowerCase().includes(lowerCaseSearchQuery) ||
          offer.tags.some(tag => tag.toLowerCase().includes(lowerCaseSearchQuery))
        );
      }
      ...
      if (locationFilter !== 'All') {
        ...
        filteredOffers = filteredOffers.filter(offer =>
          offer.location.toLowerCase() === lowerCaseLocationFilter
        );
      }
      ...
      if (genreFilter !== 'All') {
        ...
        filteredOffers = filteredOffers.filter(offer =>
          offer.genre.toLowerCase() === lowerCaseGenreFilter ||
          offer.tags.some(tag => tag.toLowerCase().includes(lowerCaseGenreFilter)) // Also search in tags for broader genre match
        );
      }

      setAllFilteredOffers(filteredOffers); 
      setLoading(false);
    }, 300);
  }, [searchQuery, locationFilter, genreFilter]); 

 
  useEffect(() => {
    const indexOfLastItem = currentPage * itemsPerPage;
    const indexOfFirstItem = indexOfLastItem - itemsPerPage;
    setCurrentOffers(allFilteredOffers.slice(indexOfFirstItem, indexOfLastItem));
  }, [currentPage, allFilteredOffers, itemsPerPage]);


  const totalPages = Math.ceil(allFilteredOffers.length / itemsPerPage);
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {currentOffers.map(offer => (
              <div
                ...
                onClick={() => onSelectBookOffer(offer.id)}
              >
